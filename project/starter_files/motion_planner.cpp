...
std::vector<State> MotionPlanner::generate_offset_goals(
    const State& goal_state) {
   // Generate "_num_paths" goals offset from the central goal by a distance of "_goal_offset".
  std::vector<State> offset_goals;

  // The offset goals will be aligned on a line perpendicular to the main goal's heading.
  // To obtain a perpendicular angle, add 90 degrees (or PI/2 radians) to the main goal's yaw.

  // Calculate the perpendicular direction by adding PI/2 to the goal yaw.
  auto perpendicular_yaw = goal_state.rotation.yaw + M_PI / 2;

  for (int i = 0; i < _num_paths; ++i) {
    auto offset_goal = goal_state;
    float offset_distance = (i - static_cast<int>(_num_paths / 2)) * _goal_offset;


    // Calculate the x and y positions for the offset goals using the "offset_distance".
    // The offset goals are positioned along a line perpendicular to the main goal's direction.
    // The direction was computed above as "perpendicular_yaw".
    offset_goal.location.x += offset_distance * std::cos(perpendicular_yaw);
    offset_goal.location.y += offset_distance * std::sin(perpendicular_yaw);


    if (valid_goal(goal_state, offset_goal)) {
      offset_goals.push_back(offset_goal);
    }
  }
  return offset_goals;
}
