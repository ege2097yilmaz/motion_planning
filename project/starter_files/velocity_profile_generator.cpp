...
double VelocityProfileGenerator::calc_distance(const double& initial_velocity,
                                                  const double& final_velocity,
                                                  const double& acceleration) {
  double distance{0.0};
  
  if (std::abs(acceleration) < DBL_EPSILON) {
    distance = std::numeric_limits<double>::infinity();
  } else {
    // Calculate the distance traveled from initial_velocity (v_i) to 
    // final_velocity (v_f) under constant acceleration/deceleration "a".
    // Ensure to account for potential division by zero.

    auto time_interval = (final_velocity - initial_velocity) / acceleration;
    
    if (time_interval < 0) { // Sign mismatch in acceleration
      distance = std::numeric_limits<double>::infinity();
    } else {
      // Use the equation: d = v_i * t + 0.5 * a * t^2
      distance = initial_velocity * time_interval + 0.5 * acceleration * std::pow(time_interval, 2);
    }
  }
...
/*
Calculate the acceleration required to achieve a given speed change over a specified distance 
using the formula: a = (v_f^2 - v_i^2) / (2 * d).

Parameters:
    v_i - initial velocity in meters per second (m/s).
    v_f - final velocity in meters per second (m/s).
    d - distance over which the speed change occurs in meters (m).
*/

double VelocityProfileGenerator::calc_acceleration(const double& initial_velocity,
                                                      const double& final_velocity,
  ...
  if (std::abs(distance) < DBL_EPSILON) {
    ...
    acceleration = (std::pow(final_velocity, 2) - std::pow(initial_velocity, 2)) / (2.0 * distance);
  }
...
/*
Calculate the final velocity after accelerating over a specified distance 
using the formula: v_f = sqrt(v_i^2 + 2ad).

Parameters:
    v_i - initial velocity in meters per second (m/s).
    a - acceleration in meters per second squared (m/s^2).
    d - distance over which the acceleration occurs in meters (m).

Ensure to handle the case where the discriminant is negative or results in infinity or NaN.
If the discriminant is negative, return zero as the final speed.
*/

double VelocityProfileGenerator::calc_final_speed(const double& initial_velocity,
                                                     const double& acceleration,
                                                     const double& distance) {
  double final_velocity{0.0};
  
  // Calculate the discriminant of the radical to determine final speed
  double discriminant = std::pow(initial_velocity, 2) + 2.0 * acceleration * distance;
  
  if (discriminant <= 0.0) {
    final_velocity = 0.0;
  } else if (discriminant == std::numeric_limits<double>::infinity() || std::isnan(discriminant)) {
    final_velocity = std::numeric_limits<double>::infinity();
  } else {
    final_velocity = std::sqrt(discriminant);
  }
